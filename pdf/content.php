<!DOCTYPE html>
<html>
    <head>
        <title>Impression to pdf</title>
        <style>
            
            table

            {
                width: 100%;
                border-collapse: collapse; /* Les bordures du tableau seront collées (plus joli) */

            }

            td, th
            {
                border: 1px solid black;
            }
                .dec{
                    margin-left: 40px;
                    margin-right: 40px;
                }
                .title{
                    margin-left: 25px;
                }
                
        </style>
    </head>
    <body>
        <div class="title">
            <h3> Fiche de compte rendu de primo installation Insulinothérapie par Pompe </h3>
        </div>
        <div class="dec">
            <h4><u>RENSEIGNEMENTS BASTIDE LE CONFORT MEDICAL</u></h4>
            <p>
                <strong>Nom IDE Responsable :</strong> nom  <br/>
                <strong>Prénom : </strong>prenom <br/>
                <strong>Mobile : </strong>mobile<br/>
                <strong>Coordonnées :</strong> Secrétariat / Astreinte 
            </p>
        </div>
        <div class="dec">
            <h4><u>RENSEIGNEMENTS PATIENT</u></h4>
            <strong>Nom :</strong> TOUT EN SAISIE LIBRE <br/>
            <strong>Prénom :</strong>prenom <br/>
            <strong>Né(e) le : </strong>date  &nbsp;&nbsp;&nbsp;&nbsp; <strong>Age :</strong> age<br/>
            <strong>Tel : </strong> tel  &nbsp;&nbsp;&nbsp;&nbsp; <strong>Mobile :</strong><br/>
            <strong>Adresse :</strong>adresse<br/>
            <strong>Code postal :</strong>code_postal<br/>    
            <strong>Ville :</strong>ville<br/>
        </div>
        <div class="dec">
            <h4><u>RENSEIGNEMENTS PRESCRIPTEUR</u></h4>
            <p>
                <strong>Etablissement prescripteur:</strong> etablissement <br/>
                <strong>Ville :</strong>   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;      <strong>Service :</strong> service <br/><br/> 

            <h4><u>Diabétologue prescripteur :</u></h4> 
            <strong>Tel :</strong> valeur      <strong>Mobile :</strong>  valeur     <strong> Ville :</strong>valeur<br/>
            <strong>Mail :</strong>valeur<br/><br/>

            <h4><u>Diabétologue libéral :</u></h4>
            <strong>Tel :</strong>  valeur    <strong> Mobile :</strong>   valeur     <strong>Ville :</strong>valeur<br/>
            <strong>Mail :</strong>valeur<br/><br/>

            <h4><u>Médecin traitant :</u></h4>
            <strong>Tel :</strong>  valeur     <strong>Mobile :</strong>  valeur      <strong>Ville :</strong>valeur<br/>
            <strong>Mail :</strong>valeur<br/><br/>

            <h4><u>IDE référente du service:</u></h4>
            <strong>Tel :</strong>  valeur     <strong>Mobile : </strong>  valeur     <strong>Ville :</strong>valeur<br/>
            <strong> Mail</strong>valeur<br/><br/>

            <h4><u>Diététicienne : </u></h4>    
            <strong>Tel :</strong>  valeur    <strong> Mobile :</strong> valeur       <strong>Ville :</strong>valeur<br/>
            <strong>Mail</strong>valeur<br/><br/>

            <strong>Membre réseau :</strong> oui              non <br/> 
            <strong>Nom du réseau</strong>valeur<br/><br/>
        </p>
    </div>
    <div class="dec">
        <h4><u>CHOIX DU MATERIEL</u></h4>
        <p>   
            <strong>Pompe :</strong>
            <strong>Date d’installation :</strong> saisie libre <br/>
            <strong>Type de cathéter :</strong> menu déroulant (droit ou tangentiel)<br/>
            <strong>Modèle :</strong> saisie libre<br/>
            <strong>Longueur de canule :</strong> saisie libre      mm              <strong> Longueur de tubulure :</strong>  saisie libre       cm
            <strong>Inserteur :</strong> oui ou non<br/><br/>

            <strong> Réservoir :</strong> saisie libre  <br/>                      
            <strong>Système de portage de la Pompe (qql lignes)</strong> saisie libre  <br/>    

            <strong>Observations et commentaires :</strong>  
        </p>

        <h4><u>DONNEES MEDICALES</u></h4>
        <p>
            <strong>Type de diabète:</strong>  DT1 ou DT2 ou Gestationnel <br/>
            <strong>Date de diagnostic :</strong> saisie libre <br/>     
            <strong>Circonstances de découverte :</strong> saisie libre   <br/>   
            <strong>Antécédents familiaux :</strong> saisie libre    <br/>  
            <strong>Date de première mise sous Pompe :</strong> saisie libre <br/>     
            <strong>HbA1C de référence avant la mise sous Pompe :</strong>   % saisie libre <br/>     
        </p><br/>
    </div>
    <div class="dec">
        <table class="table">
            <thead>
                <tr>
                    <th style="width: 25%";>Poids actuel (kg) </th>
                    <th style="width: 25%";>Poids de forme (kg)</th>
                    <th style="width: 25%";>Taille (cm)</th>
                    <th style="width: 25%";>IMC</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>valeur</td>
                    <td>valeur</td>
                    <td>valeur</td>
                    <td>valeur</td>
                </tr>
            </tbody>
        </table>
    </div>
    <br/><br/><br/>
    <div class="dec">
        <h4><u>Auto surveillance glycémique :</u></h4>
        <p>
            <strong>Nombre de glycémies par jour :</strong> menu déroulant <br/>
            <strong>Type de lecteur :</strong>   saisie libre           <strong> Date de prescription : </strong> saisie libre<br/>      
            <strong>Stylo auto piqueur/lancette :</strong> saisie libre <br/>     
            <strong>Insulinothérapie :</strong><br/>
            <strong>Nombre d’injection par jour :</strong> menu déroulant           <strong> Schéma multi injection :</strong><br/>
            <strong>Stylo :</strong> saisie libre      <strong>Autre :</strong> saisie libre         <strong> Aiguille pour stylo :</strong> saisie libre <br/>         Autre saisie <strong>libre </strong> <br/><br/><br/>    
        </p>
    </div>
    <div class="dec">
        <table class="table " >
            <thead>
                <tr>
                    <th style="width:25%";></th>
                    <th style="width:25%";>Insuline lente</th>
                    <th style="width:25%";>Insuline rapide</th>
                    <th style="width:25%";>Quantité totale</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>Matin</td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
                <tr>
                    <td>Midi</td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
                <tr>
                    <td>Soir</td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
                <tr>
                    <td>TOTAL</td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
            </tbody>
        </table>

        <h4><u>Traitements complémentaires :</u></h4>
        <table class="table">
            <thead>
                <tr>
                    <th style="width: 40%;">Spécialité</th>
                    <th style="width: 60%;">Posologie</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>valeur</td>
                    <td></td>
                </tr>
                <tr>
                    <td>valeur</td>
                    <td></td>
                </tr>
                <tr>
                    <td>valeur</td>
                    <td></td>
                </tr>
                <tr>
                    <td>valeur</td>
                    <td></td>
                </tr>
                <tr>
                    <td>valeur</td>
                    <td></td>
                </tr>
            </tbody>
        </table><br/><br/><br/>
    </div>
    <div class="dec">
        <table class="table">
            <tbody>
                <tr>
                    <td colspan=8 style="text-align: center;">Shéma basals date 1</td>
                </tr>
                <tr>
                    <td style="width:25%;" colspan=2>Schéma 1</td>
                    <td style="width:25%;" colspan=2>Schéma 2</td>
                    <td style="width:25%;" colspan=2>Schéma 3</td>
                    <td style="width:25%;" colspan=2>Schéma 4</td>
                </tr>
                <tr>
                    <td>Horaire</td>
                    <td>Débit</td>
                    <td>Horaire</td>
                    <td>Débit</td>
                    <td>Horaire</td>
                    <td>Débit</td>
                    <td>Horaire</td>
                    <td>Débit</td>
                </tr>
                <tr>
                    <td>v</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
                <tr>
                    <td>v</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
                <tr>
                    <td>v</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
            </tbody>
        </table><br/><br/><br/>
    </div>
    <div class="dec">
        <table class="table">
            <tbody>
                <tr>
                    <td colspan=8 style="text-align: center;">Shéma basals date 2</td>
                </tr>
                <tr>
                    <td style="width:25%;" colspan=2>Schéma 1</td>
                    <td style="width:25%;" colspan=2>Schéma 2</td>
                    <td style="width:25%;" colspan=2>Schéma 3</td>
                    <td style="width:25%;" colspan=2>Schéma 4</td>
                </tr>
                <tr>
                    <td>Horaire</td>
                    <td>Débit</td>
                    <td>Horaire</td>
                    <td>Débit</td>
                    <td>Horaire</td>
                    <td>Débit</td>
                    <td>Horaire</td>
                    <td>Débit</td>
                </tr>
                <tr>
                    <td>v</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
                <tr>
                    <td>v</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
                <tr>
                    <td>v</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
            </tbody>
        </table><br/><br/><br/>
    </div>
    <div class="dec">
        <table class="table">
            <thead>
                <tr>
                    <th style="width:17.66666666666667%;">Valeur glycémique</th>
                    <th style="width:17.66666666666667%;">Matin (UI)</th>
                    <th style="width:17.66666666666667%;">Midi (UI)</th>
                    <th style="width:18.66666666666667%;">Collation (UI)</th>
                    <th style="width:20%;">Soir (UI)</th>
                    <th>Autre (UI)</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>v</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
                 <tr>
                    <td>v</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
                 <tr>
                    <td>v</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
                <tr>
                    <td>v</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
                <tr>
                    <td>v</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
                <tr>
                    <td>v</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
                <tr>
                    <td>v</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
            </tbody>
        </table><br/><br/><br/>
    </div>
    <div class="dec">
        <strong>Synthèse / Commentaire : </strong> valeur <br/><br/>
        <strong>Objectif glycémique pré prandial (g/l) :</strong><br/>
        <h4><u>Hémoglobine glyquée</u></h4>
        <table class="table">
            <tbody>
                <tr>
                    <td style="width: 10%;">Dates</td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                </tr>
                <tr>
                    <td style="width: 10%;">HbA1c (%)</td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                    <td style="width: 10%;"></td>
                </tr>
            </tbody>
        </table>
    </div>
      <div class="dec">
            <h4><u>HABITUDE DE VIE</u></h4>
            
                <strong>Activité physique (type/frequence) :</strong> bla <br>
                <strong>Profession :</strong> bla <br>
                <strong>Situation familiale :</strong> bla <br>
                <strong>Observations :</strong> bla <br>
                <strong>Etat cutané au point injection :</strong> bla <br>
                <strong>Hygiène alimentaire (habitude/régime): </strong> bla <br>
                <strong>Stabilités des horraires :  </strong> oui ou non <br>
                <strong>Details: </strong> bla <br>
                <strong>Environnement social :</strong> bla <br>
                <strong>Choix des points d'injection : </strong> bla <br>
                variable ito(rougeur ou douleur ...)
           
        </div>
        <div class="dec">
            <h4><u>INSULUNOTHERAPIE FONCTIONNELLE</u></h4>
            <table class="table " >
                <thead>
                    <tr>
                        <th style="width:20%"></th>
                        <th style="width:20%">Horaires</th>
                        <th style="width:20%">Ration Insuline/glucide</th>
                        <th style="width:20%">Objectif glicémique</th>
                        <th style="width:20%">Objectif glicémique</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>Matin</td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td>Midi</td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td>Collation</td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td>Soir</td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                    </tr>

                </tbody>
            </table>
        </div>
        <div class="dec">
            <h4><u>HABITUDES ALIMENTAIRES</u></h4>
            <strong>    SUIVI DE POIDS :</strong> Oui Non <br>
            <strong>Observation : </strong> bla <br>
            <strong>Survenue de compilations :</strong> bla <br>
            <strong>ATCD/ autre pathologies :</strong> bla <br>
            <strong>Traitement :</strong> bla <br><br/><br/>
            <table class="table " >
                <tbody>
                    <tr>
                        <td style="width:10%">Date</td>
                        <td style="width:10%"></td>
                        <td style="width:10%"></td>
                        <td style="width:10%"></td>
                        <td style="width:10%"></td>
                        <td style="width:10%"></td>
                        <td style="width:10%"></td>
                        <td style="width:10%"></td>
                        <td style="width:10%"></td>
                        <td style="width:10%"></td>
                    </tr>
                    <tr>
                        <td>Taille</td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td>Poids</td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td>IMC</td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                    </tr>

                </tbody>
            </table>
        </div>
        <div class="dec">
         <font><h4><u>MANIPULATION DE LA Pompe</u></h4></font><br/>

            <table>
                    <thead>
                        <tr>
                            <td style="text-align: center; width: 15%;">Etapes</td>
                            <td style="text-align: center; width: 10%;">Date</td>
                            <td style="text-align: center; width: 10%;">Date</td>
                            <td style="text-align: center; width: 10%;">Date</td>
                            <td style="text-align: center; width: 10%;">Date</td>
                            <td style="text-align: center; width: 15%;">Commentaire</td>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>Installation de la pile</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Marche/Arrêt</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Réglage Date/heure</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr> 
                        <tr>
                            <td>Réglage du bolus</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Arrêt du bolus en cours</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Historique des bolus</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Programmation du débit basal</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Consulter les débits de base</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Programmer et arrêter le débit de base temporaire</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                    </tbody>
                </table>

                <div class="dec">
                    <h4><u>Autres : </u></h4>
                </div>

        <font><h4><u>ALARMES ET PRECAUTIONS</u></h4></font><br/>

            <table>
                    <thead>
                        <tr>
                            <td style="text-align: center; width: 25%;">Etapes</td>
                            <td style="text-align: center; width: 14%;">Date</td>
                            <td style="text-align: center; width: 14%;">Date</td>
                            <td style="text-align: center; width: 14%;">Date</td>
                            <td style="text-align: center; width: 14%;">Date</td>
                            <td style="text-align: center; width: 20%;">Commentaire</td>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>Reconnaitre une alarme</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Pile faible</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Pile épuisée</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr> 
                        <tr>
                            <td>Réservoire vide</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Occlusion/pas d'injection</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Historique des alarmes</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Entretien de la Pompe</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                    </tbody>
                </table>

 <div class="dec">
                    <h4><u>Autres : </u></h4>
</div>
        <font><h4><u>MANIPULATIONS</u></h4></font><br/>

            <table>
                    <thead>
                        <tr>
                            <td style="text-align: center; width: 25%;">Etapes</td>
                            <td style="text-align: center; width: 14%;">Date</td>
                            <td style="text-align: center; width: 14%;">Date</td>
                            <td style="text-align: center; width: 14%;">Date</td>
                            <td style="text-align: center; width: 14%;">Date</td>
                            <td style="text-align: center; width: 20%;">Commentaire</td>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>Lavage des mains</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Remplissage du réservoir</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Connexion réservoir/tubulaire</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr> 
                        <tr>
                            <td>Retour du piston</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                        <tr>
                            <td>Purge de la tubulure</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                Oui - Non - En option
                            </td>
                        </tr>
                    </tbody>
                </table>
        </div>
        <div class="dec">
    
        <h4><u>POSE DU CATHETER</u></h4><br/>

        <table>
                    
                        <tr>
                            <td style="text-align: center;"><strong>Etapes</strong></td>
                            <td style="text-align: center;"><strong>Date</strong></td>
                            <td style="text-align: center;"><strong>Date</strong></td>
                            <td style="text-align: center;"><strong>Date</strong></td>
                            <td style="text-align: center;"><strong>Date</strong></td>
                            <td style="text-align: center;"><strong>Commentaire</strong></td>
                        </tr>
                    
                    <tbody>
                        <tr>
                            <td><strong>Mise en place du cathéter dans l'Inserteur</strong></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>Retrait de l'adhésif et de la protection de l'aiguille</strong></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>Armement de l'Inserteur</strong></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                        </tr> 
                        <tr>
                            <td><strong>Choix du lieu d'injection</strong></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>Désinfection cutanée</strong></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>Déclenchement de l'Inserteur</strong></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>Retrait de l'aiguille</strong></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>Amorce de la canule</strong></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>Contrôle glycémique 2h après la pose</strong></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>Conservation de l'insuline</strong></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>Fréquence de changement des cathéters</strong></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td></td>
                        </tr>
                    </tbody>
                </table>
        

        
            <h4><u>CONNAISSANCE</u></h4><br/>

                <table>
                    
                        <tr>
                            <td style="text-align: center;"></td>
                            <td style="text-align: center;">OUI / NON</td>
                            <td style="text-align: center;"><strong>Observation</strong></td>                            
                        </tr>
                    
                    
                        <tr>
                            <td><strong>Numéro de l'astreinte médicale</strong></td>
                            <td></td>
                            <td></td>                            
                        </tr>
                        <tr>
                            <td><strong>Numéro de l'astreinte BLCM</strong></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr> 
                            <td>
                                <strong>Kit de sécurité</strong><br/>
                                <strong>&nbsp;&nbsp;&nbsp;&nbsp; * &nbsp;&nbsp;&nbsp;Stylo d'insuline rapide</strong><br/>
                                <strong>&nbsp;&nbsp;&nbsp;&nbsp; * &nbsp;&nbsp;&nbsp;Stylo d'insuline lente</strong><br/>
                                <strong>&nbsp;&nbsp;&nbsp;&nbsp; * &nbsp;&nbsp;&nbsp;Aiguilles</strong><br/>
                                <strong>Matériel de secours</strong>
                            </td>
                           
                            <td>
                                <br/>
                                    * <br/>
                                    * <br/>
                                    *   
                            </td>
                            
                            <td>
                                <strong>Date de péremption</strong> <br/>
                                    * <br/>
                                    *
                            </td>
                        
                        </tr> 
                        <tr>
                            <td><strong>Schéma de remplacement</strong></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>CAT en cas d'hypoglycémie</strong></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>Maitrise du ressucrage</strong></td>
                            <td></td>
                            <td></td>
                        </tr>                        
                        <tr>
                            <td><strong>CAT en cas d'hyperglycémie</strong></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>Maitrise de l'acceptation du bolus de correction</strong></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>CAT en cas d'accétonémie</strong></td>
                            <td></td>
                            <td></td>
                        </tr>                     
                    
                </table>
        

        
            <h4><u>AUTO SURVEILLANCE GLYCEMIQUE</u></h4><br/>
                <table>
                    
                        <tr>
                            <td></td>
                            <td style="text-align: center;"><strong>Détails</strong></td>
                            <td style="text-align: center;"><strong>Observations</strong></td>                            
                        </tr>
                    
                    <tbody>                      
                        <tr>
                            <td><strong>Nom du dispositif ASG principal et des bandelettes</strong></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>Nom du dispositif ASG secondaire éventuel et des bandelettes</strong></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td><strong>Nom du dispositif auto piqueur</strong></td>
                            <td></td>
                            <td></td>
                        </tr>                        
                        <tr>
                            <td><strong>Etat cutané des doigts/respect de "la pince"</strong></td>
                            <td></td>
                            <td></td>
                        </tr>                                        
                    </tbody>
                </table>
        

        
            <h4><u>AUTO SURVEILLANCE GLYCEMIQUE (ASG)</u></h4><br/>
                <table>
                   
                        <tr>
                            <td style="text-align: center;"></td>
                            <td style="text-align: center;"><strong>Prescription</strong></td>
                            <td style="text-align: center;"><strong>Réalisation réelle</strong></td>                            
                        </tr>
                   
                    <tbody>                      
                        <tr>
                            <td><strong>Fréqeunce des glycémies</strong></td>
                            <td></td> 
                            <td></td>
                        </tr>
                        <tr>
                            <td style="text-align: center;"></td>
                            <td style="text-align: center;"><strong>OUI</strong></td>
                            <td style="text-align: center;"><strong>NON</strong></td>
                        </tr>
                        <tr>
                            <td><strong>Connaissance et respect des mesures d'hygiène relatives l'ASG</strong></td>
                            <td></td>
                            <td></td>
                        </tr>                                      
                    </tbody>
                </table>
        

    </div>

</body>
</html>